"""Thread-scoped session adapter.

Layer: Infrastructure.

Wraps a base ``Session`` and forwards every call to a per-thread override
when active, falling back to the base otherwise. The override is a
``threading.local`` because ``ContextVar`` does not propagate to worker
threads spawned by ``ThreadPoolExecutor`` (the GEPA evaluation model).
"""
